...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  ClearSegDefs - initiated from Make"
  echo "  ClearSegDefs - initiated from Make"
else
  ~/sysprocs/LOGMSG "  ClearSegDefs - initiated from Terminal"
  echo "  ClearSegDefs - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
#	2/15/23.	wmk.
#	3/5/23.		wmk.	change *P1 to 808 so DoSedSegDefs edits properly.
echo "WARNING: you are about to clear the segment definitions"
read -p " for territory 808... continue (y/n)? "
yn=${REPLY^^}
if [ "$yn" == "N" ];then
 echo "  Review TerrID.db segment definitions for territory 808."
 echo "   ClearTerrSegs abandoned at user request."
 exit 1
else
 echo "  Proceeding to clear segment definitions for territory 808... "
fi
