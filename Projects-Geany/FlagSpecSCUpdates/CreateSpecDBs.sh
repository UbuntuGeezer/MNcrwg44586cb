...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  CreateSpecDBs - initiated from Make"
  echo "  CreateSpecDBs - initiated from Make"
else
  ~/sysprocs/LOGMSG "  CreateSpecDBs - initiated from Terminal"
  echo "  CreateSpecDBs - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  CreateSpecDBs complete."
~/sysprocs/LOGMSG "  CreateSpecDBs complete."
#end proc
