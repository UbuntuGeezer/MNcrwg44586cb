--UpdtSpecSCBridge.psq/sql - Update SC Bridge table for <special-db>.
--	4/25/22.	wmk.
--
...
-- *					<special-db>.db.Spec_SCBridge; @ @ and z z used
...
-- *	<special-db>.db - as db29, Special download of SC records
-- *		Spec_SCBridge - Bridge formatted records extracted from SCPA
-- *			for street/area <special-db>.
...
ATTACH '$pathbase'
 ||		'/RawData/SCPA/SCPA-Downloads/Special/WhitePineTreeRd.db'
  AS db29;
-- * PRAGMA db29.table_info(Spec_SCBridge);
-- * PRAGMA db29.table_info(DiffSpec);
-- * Now Update records using information from DiffSpec.;
