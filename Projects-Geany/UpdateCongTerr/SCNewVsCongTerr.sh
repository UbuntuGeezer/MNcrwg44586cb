...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  SCNewVsCongTerr - initiated from Make"
  echo "  SCNewVsCongTerr - initiated from Make"
else
  ~/sysprocs/LOGMSG "  SCNewVsCongTerr - initiated from Terminal"
  echo "  SCNewVsCongTerr - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  SCNewVsCongTerr complete."
~/sysprocs/LOGMSG "  SCNewVsCongTerr complete."
#end proc
