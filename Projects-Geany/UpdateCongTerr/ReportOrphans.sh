...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  ReportOrphans - initiated from Make"
  echo "  ReportOrphans - initiated from Make"
else
  ~/sysprocs/LOGMSG "  ReportOrphans - initiated from Terminal"
  echo "  ReportOrphans - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  ReportOrphans complete."
~/sysprocs/LOGMSG "  ReportOrphans complete."
#end proc
