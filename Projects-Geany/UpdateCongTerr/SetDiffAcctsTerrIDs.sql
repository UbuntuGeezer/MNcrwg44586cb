...
-- *	5/27/22.	wmk.
-- *
-- * Entry.  SCPA-Downloads/SCPADiff_05-26.db is differences database.
...
-- *;
.open '$pathbase/RawData/SCPA/SCPA-Downloads/SCPADiff_05-26.db'
attach '$pathbase'
 || '/DB-Dev/MultiMail.db'
 AS db3;
attach '$pathbase'
...
CONGTERRID FROM db3.SplitProps
 WHERE Acct
 IN (SELECT "Account#" FROM Diff0526))
UPDATE DiffAccts
SET TerrID =
CASE 
