#!/bin/bash
echo " ** AllIntegrateOldDiffs.sh out-of-date **";exit 1
echo " ** AllIntegrateOldDiffs.sh out-of-date **";exit 1
# 9/23/22.    wmk.   (automated) CB *codebase env var support.
# AllIntegrateOldDiffs.sh = Update RU downloads from list.
# 5/5/22.	wmk.
#
# Usage.	bash AllIntegrateOldDiffs.sh <state> <county> <congno>
#
...
# Legacy mods.
# 7/6/21.	wmk.	original code; adapted from AllExtractOldDiffs;
#			 includes multihost.
#
...
if [ "$P1$P2$P3" != "$congterr" ];then
 ...
 echo "** $P1$P2$P3 mismatch with *congterr* - AllIntegrateOldDiffs abandoned **"
 exit 1
fi
# handle case where called from Make.
if [ -z "$system_log" ]; then
  system_log=$folderbase/ubuntu/SystemLog.txt
  ~/sysprocs/LOGMSG "   AllIntegrateOldDiffs initiated from Make."
else
  ~/sysprocs/LOGMSG "   AllIntegrateOldDiffs initiated from Terminal."
fi
...
MY_PROJ='Territories'
echo "  AllIntegrateOldDiffs beginning processing."
...
#  echo -e "  $firstchar\n is first char of line." >> $HOME/temp/scratchfile
  ...
  if [ "$firstchar" = "#" ]; then			# skip comment
   ...
   TID=${REPLY:0:len}
   bash ~/sysprocs/LOGMSG "  IntegrateOldDiffs $TID initiated."
   ./DoSed.sh $TID
   ./IntegrateOldDiffs.sh $TID
...
# end proc body
~/sysprocs/LOGMSG " AllIntegrateOldDiffs complete." >> $system_log
echo " 	AllIntegrateOldDiffs complete."
