...
#			if <terrid> is a mobile home park terrritory 
#			 Bay Indies (235-251, 268,269)
#			 Ridgewood (261-264)
#			 Bay Lake (317-321)
#			or a letter/phone territory (601-647)
#			DoSed will flag an error and prompt the user to use either
#			 the UpdateMHPDwnld or UpdateLtrDwnld project, as appropriate.
#
#	Entry.
...
# ---------------------
# 2/5/23.	wmk.	reject MHP and Letter territories.
...
# reject OBSOLETE territories.
if test -f $pathbase/$rupath/Terr$P1/OBSOLETE;then
 echo " ** Territory $P1 OBSOLETE - UpdateRUDwnld/DoSed exiting..**"
 exit 2
fi
...
# reject Letter territories, since they have special update rules.
if [[ ( $P1 -ge 601  &&  $P1 -le 647 ) ]];then
 echo " ** $P1 is a Letter territory - use the UpdateLtrDwnld project **"
 exit 1
fi
...
if test -f $pathbase/$rupath/$Terryyy/OBSOLETE;then
 echo " ** Territory yyy OBSOLETE - UpdateRUDwnld/DoSed exiting...**"
 exit 2
fi
projpath=$codebase/Projects-Geany/UpdateRUDwnld
# 
echo "s/yyy/$P1/g;s/yyy/$P1/g" > sedative.txt
#read -p "pausing... examine $PWD/sedative.txt..."
