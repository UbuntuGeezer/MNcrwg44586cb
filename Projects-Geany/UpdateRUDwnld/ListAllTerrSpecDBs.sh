#!/bin/bash
echo " ** ListAllTerrSpecDBs.sh out-of-date **";exit 1
echo " ** ListAllTerrSpecDBs.sh out-of-date **";exit 1
# ListAllTerrSpecDBs.sh = List all /Special db,s used by territories in list.
# 	7/2/23.	wmk.
#
# Usage. bash	 ListAllTerrSpecDBs.sh [<tidlist>]
#
...
# ---------------------
# 6/29/23.	wmk.	original code; adapted from AllTerrDates.
# 7/2/23.	wmk.	alldbSQLs removed at startup.
# Legacy mods.
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "   ListAllTerrSpecDBs initiated from Make."
else
  ~/sysprocs/LOGMSG "   ListAllTerrSpecDBs initiated from Terminal."
fi
...
pushd ./ > /dev/null
cd $projpath
echo "  ListAllTerrSpecDBs beginning processing."
error_counter=0		# set error counter to 0
IFS="&"			# set & as the word delimiter for read.
if test -f $TEMP_PATH/alldbSQLs.txt;then rm $TEMP_PATH/alldbSQLs.txt;fi
if [ $onlyone -eq 0 ];then
 file=$P1
 i=0
 while read -e; do
  #reading each line
...
#  echo -e "  $firstchar\n is first char of line." >> $TEMP_PATH/scratchfile
  ...
  if [ "$firstchar" == "#" ]; then			# skip comment
   ...
   if test -f $pathbase/$rupath/Terr$TID/OBSOLETE;then
    ...
    ./ListTerrSpecDBs.sh $TID >> $TEMP_PATH/alldbSQLs.txt
   fi		# end skip
...
# cat $TEMP_PATH/alldbSQLs.txt
 mawk '/^Terr/ {print}' $TEMP_PATH/alldbSQLs.txt
else	# handle only 1 territory
 TID=$P1
 $projpath/ListTerrSpecDBs.sh $TID
fi
popd > /dev/null
# endprocbody
~/sysprocs/LOGMSG " ListAllTerrSpecDBs complete."
echo " 	ListAllTerrSpecDBs complete."
