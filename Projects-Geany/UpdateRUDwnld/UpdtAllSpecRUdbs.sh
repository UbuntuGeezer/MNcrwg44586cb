#!/bin/bash
echo " ** UpdtAllSpecRUdbs.sh out-of-date **";exit 1
echo " ** UpdtAllSpecRUdbs.sh out-of-date **";exit 1
# UpdtAllSpecRUdbs.sh = Update RU downloads from list.
# 	6/6/23.	wmk.
#
# Usage.	bash UpdtAllSpecRUdbs.sh
#
...
# echo "UpdtAllSpecRUdbs <state> <county> <congno> missing parameter(s) - abandoned."
# exit 1
#fi
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "   UpdtAllSpecRUdbs initiated from Make."
else
  ~/sysprocs/LOGMSG "   UpdtAllSpecRUdbs initiated from Terminal."
fi
...
MY_PROJ='Territories'
echo "  UpdtAllSpecRUdbs beginning processing."
...
#  echo -e "  $firstchar\n is first char of line." >> $TEMP_PATH/scratchfile
  ...
  if [ "$firstchar" = "#" ]; then			# skip comment
   ...
   if test -f $pathbase/$rupath/Terr$TID/OBSOLETE;then
    skip=1
    echo " ** Territory $TID OBSOLETE - UpdateAllSpecRUdbs skipping..**"
   fi	# end OBSOLETE
   if [ $skip -eq 0 ];then
...
# endprocbody
~/sysprocs/LOGMSG " UpdtAllSpecRUdbs complete." >> $system_log
echo " 	UpdtAllSpecRUdbs complete."
