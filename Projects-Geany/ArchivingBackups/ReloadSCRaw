#!/bin/bash
# ReloadSCRaw.sh  - Reload file(s) from archive of SCRaw subdirectories.
# 11/17/22.	wmk.
#
#	Usage. bash ReloadSCRaw <filelist> [-o] [-u mountname] [<statecountycongo>]
#
...
#
# ReloadSCRaw.sh performs a *tar* reload of the
...
if [ -z "$P1" ];then
 echo "ReloadSCRaw <filespec> [-u|-ou|-uo <mountname>] [<state><county><congno>] missing parameter(s) - abandoned."
...
if [ -z "$P2" ] || [ -z "$P3" ];then
  echo "ReloadSCRaw <filespec> -u <mountname> [<state><county><congno>] missing parameter(s) - abandoned."
  read -p "Enter ctrl-c to remain in Terminal:"
  exit 1
fi
...
if [ ! -z "$P2" ];then
 ...
 if [ "$P2" != "-u" ] && [ "$P2" != "-ou" ] && [ "$P2" != "-uo" ]&& [ "$P2" != "-o" ];then
  echo "ReloadSCRaw <filespec> [-u <mountname>] [<state><county><congo>] unrecognized '-' option - abandoned."
 fi
 if [ "${P2:1:1}" == "u" ] || [ "${P2:2:1}" == "u" ] ;then
  if ! test -d $U_DISK/$P3;then
   ...
   yq=${REPLY^^}
   if [ "$yq" == "Q" ];then
    echo "  ReloadSCRaw abandoned, drive not mounted."
    sysprocs/LOGMSG "  ReloadSCRaw abandoned, drive not mounted."
    read -p "Enter ctrl-c to remain in Terminal:"
    exit 0
   fi
  fi
...
# echo "** $P1$P2$P3 mismatch with *congterr* - ReloadSCRaw abandoned **"
# exit 1
#fi
# debugging code...
echo "P1 = : $P1"
...
echo "isAll = $isAll"
echo "P4 = : $P4"
echo "ReloadSCRaw parameter tests complete"
#read -p "Enter ctrl-c to remain in Terminal:"
#exit 0
# end debugging code.
#
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "   ReloadSCRaw $P1 $P2 $P3 $P4 initiated from Make."
  echo "   ReloadSCRaw initiated."
else
  ~/sysprocs/LOGMSG "   ReloadSCRaw $P1 $P2 $P3 $P4 initiated from Terminal."
  echo "   ReloadSCRaw initiated."
fi
...
cd $pathbase
  while read -e;do
   ...
   if [ $isAll -eq 0 ];then
    ...
    --wildcards  \
    $writeover \
    RawData/SCPA/SCPA-Downloads/$P1
  else
    tar --extract \
    --file=$dump_path/$P4$rawdatasc.$REPLY.tar \
    --wildcards  \
