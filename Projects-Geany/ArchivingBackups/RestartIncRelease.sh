#!/bin/bash
# RestartRelease.sh  - Set up for fresh incremental archiving of Release.
#	4/15/23.	wmk.
#
#	Usage. bash RestartRelease.sh <state> <county> <congno> -u <mount-name>
#
#		<state> = 2 char state abbreviation
#		<conty> = 4 char county abbreviation
#		<congno> = congregation number
#		-u = use USB removable device
...
#
# Exit Results.
#	/Territories/Release.nnn.tar files all deleted after warning;
#	/Territories/log/Release.snar-0 deleted after warning;
#	/Territories/log/Releaselevel.txt deleted; this sets up next run of
#	IncDumpRelease to start at level 0;
#
# Modification History.
# ----------------------
# 4/15/23.	wmk.	-u, <mount-name> support.
# Legacy mods.
# 4/22/22.	wmk.	modified for general use /FL/SARA/86777.
# 4/23/22.	wmk.	*congterr* env var introduced.
# 4/25/22.	wmk.	*congterr* used througout.
...
if [ -z "$P1" ] || [ -z "$P2" ] || [ -z "$P3" ] || [ -z "$P4"  || [ -z "$P5" ];then
 echo "RestartIncRelease <state> <county> <congno> -u <mount-name> missing parameter(s) - abandoned."
 exit 1
fi
if [ "$P1$P2$P3" != "$congterr" ];then
 echo "*congterr* = :$congterr"
 echo "** $P1$P2$P3 mismatch with *congterr* - RestartIncRelease abandoned **"
 exit 1
fi
 if [ "$P4" != "-U" ];then
  echo "RestartIncRelease <state> <county> <congno> [-u <mount-name>] $4 unrecognized option - abandoned."
 ...
 if ! test -d $U_DISK/$P5;then
  ...
  yn=${REPLY^^}
  if [ "$yn" != "Y" ];then
   echo "RestartIncRelease abandoned at user request."
   exit 1
  else
   if ! test -d $U_DISK/$P5;then
    echo "$P5 still not mounted - RestartIncRelease abandoned."
    exit 1
   else
...
if [ -z "$system_log" ]; then
  ...
  bash ~/sysprocs/LOGMSG "   RestartIncRelease initiated from Make."
  echo "   RestartRelease initiated."
else
  bash ~/sysprocs/LOGMSG "   RestartIncRelease initiated from Terminal."
  echo "   RestartRelease initiated."
fi
TEMP_PATH=$folderbase/temp
if [ "$NOPROMPT" == "" ];then NOPROMPT=0;fi;if [ $NOPROMPT -ne 1 ];then
 echo " **WARNING - proceeding will remove all prior Release incremental dump files!**"
 read -p "OK to proceed (Y/N)? "
 ynreply=${REPLY,,}
 if [ "$ynreply" == "y" ];then
  echo "  Proceeding to remove prior incremental dump files..."
 else
  ~/sysprocs/LOGMSG "  User halted RestartIncRelease."
   echo " Stopping RestartIncRelease - secure Release incremental backups.."
   exit 0
 fi
fi	# end NOPROMPT
#
...
#endprocbody
~/sysprocs/LOGMSG "  RestartIncRelease $P4 $P5 complete."
echo "  RestartIncRelease $P4 $P5 complete."
# end RestartReleaseData
