#!/bin/bash
# IncDumpBRURaw.sh  - Incremental archive of BRawData/RefUSA subdirectories.
# 5/11/22.	wmk.
#
#	Usage. bash IncDumpBRURaw.sh 
#
# Dependencies.
#	~/Territories/BRawData/RefUSA/RefUSA-Downloads - base directory for Terrxxx folders with
...
#
# Notes. IncDumpBRURaw.sh performs an incremental archive (tar) of the
...
# reset the RawData dump information so that the next IncDumpBRURaw
...
# subsequent IncDumpBRURaw calls. The initial archive file is named
# archive.0.tar.
...
if [ -z "$P1" ] || [ -z "$P2" ] || [ -z "$P3" ];then
 echo "IncDumpBRURaw <state> <county> <congno> missing parameter(s) - abandoned."
 exit 1
fi
...
if [ -z "$system_log" ]; then
  ...
  bash ~/sysprocs/LOGMSG "   IncDumpBRURaw initiated from Make."
  echo "   IncDumpBRURaw initiated."
else
  bash ~/sysprocs/LOGMSG "   IncDumpBRURaw initiated from Terminal."
  echo "   IncDumpBRURaw initiated."
fi
TEMP_PATH="$HOME/temp"
#
...
if ! test -f $pathbase/log/$rawdata$level.txt;then
 ...
 if ! test -d $pathbase/log;then
  ...
  archname=$congterr$rawdata.0.tar
  echo $archname
  ...
  tar --create \
	  ...
	  --file=$archname \
	  BRawData/RefUSA
  ~/sysprocs/LOGMSG "  IncDumpBRURaw $P1 $P2 $P3 complete."
  echo "  IncDumpBRURaw $P1 $P2 $P3 complete"
  exit 0
fi
...
jumpto EndProc
EndProc:
~/sysprocs/LOGMSG "  IncDumpBRURaw complete."
echo "  IncDumpBRURaw complete"
# end IncDumpBRURaw
