...
# 9/8/21.	wmk.	added optional P1 target drive/folder spec since
#					dump sizes now exceed capacity of system flash
#					drive; documentation review and minor corrections.
#
...
# dump is performed. A shell utility RestartRawData.sh is provided to
# reset the RawData dump information so that the next IncDumpRawData
...
if ! test -f ./log/Rawlevel.txt;then
 ...
 if ! test -d ./log;then
  ...
  if [ -z "$P1" ];then
   ...
   tar --create \
	  --listed-incremental=./log/RawData.snar-0 \
	  --file=$P1/RawData.0.tar \
	  RawData
  fi
  notify-send "IncDumpRawData" " $P1 complete."
