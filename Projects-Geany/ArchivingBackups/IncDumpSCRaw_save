...
#		  specified, will be dumped to local hard disk *pathbase; if specified
#		  *dumpbase will be set pointing to the flashdrive
#
# Dependencies.
...
# RawData subdirectories. If the file $dumpbase/log/RawDataSC.snar-0 does not exist
...
# The file $dumpbase/log/RawDataSC.snar is created as the listed-incremental archive
# information. The file $dumpbase/log/RawSClevel.txt is intialized with "0". This file
...
# archive.0.tar.
# If the $dumpbase/log folder exists under RawData a level-1 incremental dump
...
# next level # obtained by incrementing $dumpbase/log/level.txt. tar will be
...
if ! test -f $dumpbase/log/$congterr$rawsc$level.txt;then
 ...
 if ! test -d $dumpbase/log;then
  ...
  if [ "$USER" != "vncwmk3" ];then
   notify-send "IncDumpSCRaw" " $P1 $P2 $P3 complete."
  fi
