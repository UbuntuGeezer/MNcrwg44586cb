#!/bin/bash
# ReloadAllSpecSC.sh  - Reload file(s) from archive of SCRaw subdirectories.
# 9/26/22.	wmk.
#
#	Usage. bash ReloadAllSpecSC  -u|-ou|-uo <mountname> [<statecountycongo>]
#
...
#							 default *congterr
#
# Entry. SpecSCdblist.txt = list of Special .dbs to reload
#
...
#
# Exit. ReloadSpecSC.sh called for all Special dbs in SpecSCdblist.txt
#	SCPA-Downloads/Special/*.db reloaded; overwritten if -o specified.
#
# Modification History.
# ---------------------
# 9/26/22.	wmk.	original code; adapted from ReloadSpecSC.
# Legacy  mods.
...
#
# ReloadAllSpecSC.sh performs a *tar* reload of the
...
if [ -z "$P1" ] || [ -z "$P2" ];then
 echo "ReloadAllSpecSC -u|-ou|-uo <mountname> [<state><county><congno>] missing parameter(s) - abandoned."
 exit 1
fi
# to specify P3, P1, P2 must be present, and may not be empty strings.
if  [ ! -z "$P1" ] && $([ "${P1:1:1}" == "u" ] || [ "${P1:2:1}" == "u" ]) && [ -z "$P2" ];then
  echo "ReloadAllSpecSC  -u <mountname> [<state><county><congno>] missing parameter(s) - abandoned."
  echo "P1 = : $P1"		# -u|-uo|-ou
...
if [ ! -z "$P1" ];then
 if [ "$P1" != "-u" ] && [ "$P1" != "-ou" ] && [ "$P1" != "-uo" ]];then
  echo "ReloadAllSpecSC [-u <mountname>] [<state><county><congo>] unrecognized '-' option - abandoned."
 fi
 if [ "${P1:1:1}" == "u" ] || [ "${P1:2:1}" == "u" ] ;then
  if ! test -d $U_DISK/$P2;then
   ...
   yq=${REPLY^^}
   if [ "$yq" == "Q" ];then
    echo "  ReloadAllSpecSC abandoned, drive not mounted."
    sysprocs/LOGMSG "  ReloadAllSpecSC abandoned, drive not mounted."
    exit 0
   fi
  fi
 fi
...
# echo "** $P1$P2$P3 mismatch with *congterr* - ReloadAllSpecSC abandoned **"
# exit 1
#fi
# debugging code...
echo "P1 = : $P1"
echo "P2 = : $P2"
echo "P3 = : $P3"
echo "ReloadAllSpecSC parameter tests complete"
#exit 0
# end debugging code.
#
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "   ReloadAllSpecSC $P1 $P2 $P3 $P4 initiated from Make."
  echo "   ReloadAllSpecSC initiated."
else
  ~/sysprocs/LOGMSG "   ReloadAllSpecSC $P1 $P2 $P3 $P4 initiated from Terminal."
  echo "   ReloadAllSpecSC initiated."
fi
TEMP_PATH="$folderbase/temp"
#
#proc body here
# loop on SpecSCdblist.txt calling ReloadSpecSC.sh with parameters set.
projpath=$codebase/Projects-Geany/ArchivingBackups
file=$projpath/SpecSCdblist.txt
filecnt=0
while read -e;do
 dbname=$REPLY.db
 frstchar=${REPLY:0:1}
 ...
 $projpath/ReloadSpecSC.sh $dbname $P1 $P2
 filecnt=$((filecnt+1))
done < $file
echo "  $filecnt files processed."
~/sysprocs/LOGMSG "  ReloadAllSpecSC $P1 $P2 complete."
echo "  ReloadAllSpecSC $P1 $P2 complete."
exit 0
# end ReloadAllSpecSC.sh
