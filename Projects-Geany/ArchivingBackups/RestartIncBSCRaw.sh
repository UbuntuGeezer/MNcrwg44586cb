#!/bin/bash
# RestartBSCRaw.sh  - Set up for fresh incremental archiving of BSCRaw.
# 	4/23/22.	wmk.
#
#	Usage. bash RestartBSCRaw.sh
#
# Dependencies.
...
#
# Exit Results.
#	/Territories/BSCRaw.nnn.tar files all deleted after warning;
#	/Territories/log/BSCRaw.snar-0 deleted after warning;
#	/Territories/log/BSCRawlevel.txt deleted; this sets up next run of
#	IncDumpBSCRaw to start at level 0;
#
...
if [ -z "$P1" ] || [ -z "$P2" ] || [ -z "$P3" ];then
 echo "RestartIncBSCRaw <state> <county> <congno> missing parameter(s) - abandoned."
 exit 1
fi
...
if [ -z "$system_log" ]; then
  ...
  bash ~/sysprocs/LOGMSG "   RestartIncBSCRaw initiated from Make."
  echo "   RestartBSCRaw initiated."
else
  bash ~/sysprocs/LOGMSG "   RestartIncBSCRaw initiated from Terminal."
  echo "   RestartBSCRaw initiated."
fi
TEMP_PATH=$folderbase/temp
echo " **WARNING - proceeding will remove all prior BSCRaw incremental dump files!**"
read -p "OK to proceed (Y/N)? "
ynreply=${REPLY,,}
if [ "$ynreply" == "y" ];then
  echo "  Proceeding to remove prior incremental dump files..."
else
  ~/sysprocs/LOGMSG "  User halted RestartIncBSCRaw."
  echo " Stopping RestartIncBSCRaw - secure BSCRaw incremental backups.."
  exit 0
fi
#
...
if [ "$USER" != "vncwmk3" ];then
 notify-send "RestartIncBSCRaw" " $1 complete."
fi
~/sysprocs/LOGMSG "  RestartIncBSCRaw $P1 complete."
echo "  RestartIncBSCRaw $P1 complete"
# end RestartBSCRawData
