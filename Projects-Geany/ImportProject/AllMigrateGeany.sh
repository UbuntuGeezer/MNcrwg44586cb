...
# 4/13/22.	wmk.	folderbase redefinition conditional; *pathbase* support.
# 5/15/22.  wmk.    (automated) pathbase corrected to TX/HDLG/99999.
#
# This shell migrates .geany files from the HP environment running a
# non-persistent Linux/Ubuntu to a ChromeBook Linux environment.
...
if [ $local_debug = 1 ]; then
 ...
 cd $TEMP_PATH		# write files to temp folder
fi
#proc body here
pushd ./ >> $TEMP_PATH/bitbucket.txt
cd $codebase/Projects-Geany/CBtoHPgeany
MY_PROJ='Territories'
...
~/sysprocs/LOGMSG " AllMigrateGeany $P1 complete."
