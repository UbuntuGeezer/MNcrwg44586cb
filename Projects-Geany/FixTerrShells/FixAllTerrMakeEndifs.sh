#!/bin/bash
echo " ** FixAllTerrMakeEndifs.sh out-of-date **";exit 1
echo " ** FixAllTerrMakeEndifs.sh out-of-date **";exit 1
# 9/23/22.    wmk.   (automated) CB *codebase env var support.
# FixAllTerrMakeEndifs.sh - fix all Makefiles with *pathbase* support.
# 5/28/22.    wmk.   (automated) pathbase corrected to TX/HDLG/99999.
# 5/15/22.    wmk.   (automated) pathbase corrected to FL/SARA/86777.
#	5/9/22.	wmk.
#
# Usage.  FixAllTerrMakeEndifs.sh <sqlpath>
#
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  FixAllTerrMakeEndifs - initiated from Make"
  echo "  FixAllTerrMakeEndifs - initiated from Make"
else
  ~/sysprocs/LOGMSG "  FixAllTerrMakeEndifs - initiated from Terminal"
  echo "  FixAllTerrMakeEndifs - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
# proc body here.
# get file list...
...
#end proc body.
~/sysprocs/LOGMSG "  FixAllTerrMakeEndifs complete."
echo "  FixAllTerrMakeEndifs complete."
# end FixAllTerrMakeEndifs.sh
