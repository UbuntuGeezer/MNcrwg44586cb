...
# 6/14/23.	wmk.
#
# Usage. bash  GetDelList.sh <terrid>
#
...
P1=$1
if [ -z "$P1" ];then
 echo "GetDelList <terrid> missing parameter(s) - abandoned."
 exit 1
fi
listall=0
if [ "$P1" == "!" ];then
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  GetDelList - initiated from Make"
  echo "  GetDelList - initiated from Make"
else
  ~/sysprocs/LOGMSG "  GetDelList - initiated from Terminal"
  echo "  GetDelList - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
#procbodyhere
echo "-- * GetDelList.sql - Get DNC property ID given UnitAddress, Unit."  > SQLTemp.sql
...
echo ".output '$TEMP_PATH/DeletedList.csv'"  >> SQLTemp.sql
echo "SELECT * " >> SQLTemp.sql
echo "FROM DeletedDNCs"  >> SQLTemp.sql
...
#endprocbody
sqlite3 < SQLTemp.sql
echo " Deleted DNC list on $TEMP_PATH/DeletedList.csv"
echo "  GetDelList complete."
~/sysprocs/LOGMSG "  GetDelList complete."
# end GetDelList.sh
