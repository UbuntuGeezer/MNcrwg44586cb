...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  DumpTerrLog - initiated from Make"
  echo "  DumpTerrLog - initiated from Make"
else
  ~/sysprocs/LOGMSG "  DumpTerrLog - initiated from Terminal"
  echo "  DumpTerrLog - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  DumpTerrLog complete."
~/sysprocs/LOGMSG "  DumpTerrLog complete."
#end proc
