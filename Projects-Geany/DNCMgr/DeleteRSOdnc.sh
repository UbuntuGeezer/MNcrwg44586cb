...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  DeleteRSOdnc - initiated from Make"
  echo "  DeleteRSOdnc - initiated from Make"
else
  ~/sysprocs/LOGMSG "  DeleteRSOdnc - initiated from Terminal"
  echo "  DeleteRSOdnc - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  DeleteRSOdnc complete."
~/sysprocs/LOGMSG "  DeleteRSOdnc complete."
#end proc
