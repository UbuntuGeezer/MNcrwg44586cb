...
# 6/14/23.	wmk.
#
# Usage. bash  GetDNCList.sh <terrid>
#
...
P1=$1
if [ -z "$P1" ];then
 echo "GetDNCList <terrid> missing parameter(s) - abandoned."
 exit 1
fi
getall=0
if [ "$P1" == "!" ];then
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  GetDNCList - initiated from Make"
  echo "  GetDNCList - initiated from Make"
else
  ~/sysprocs/LOGMSG "  GetDNCList - initiated from Terminal"
  echo "  GetDNCList - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
#procbodyhere
echo "-- * GetDNCList.sql - Get DNC property ID given UnitAddress, Unit."  > SQLTemp.sql
...
echo ".output '$TEMP_PATH/DNCList.txt'"  >> SQLTemp.sql
...
#endprocbody
sqlite3 < SQLTemp.sql
echo " DNC List is on $TEMP_PATH/DNCList.txt"
echo "  GetDNCList complete."
~/sysprocs/LOGMSG "  GetDNCList complete."
# end GetDNCList.sh
