...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  DumpDNCLog - initiated from Make"
  echo "  DumpDNCLog - initiated from Make"
else
  ~/sysprocs/LOGMSG "  DumpDNCLog - initiated from Terminal"
  echo "  DumpDNCLog - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  DumpDNCLog complete."
~/sysprocs/LOGMSG "  DumpDNCLog complete."
#end proc
