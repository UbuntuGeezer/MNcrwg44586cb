...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  Purgatory - initiated from Make"
  echo "  Purgatory - initiated from Make"
else
  ~/sysprocs/LOGMSG "  Purgatory - initiated from Terminal"
  echo "  Purgatory - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  Purgatory complete."
~/sysprocs/LOGMSG "  Purgatory complete."
#end proc
