...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  PurgeTerrSpec - initiated from Make"
  echo "  PurgeTerrSpec - initiated from Make"
else
  ~/sysprocs/LOGMSG "  PurgeTerrSpec - initiated from Terminal"
  echo "  PurgeTerrSpec - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
sqlite3 < SQLTemp.sql
echo "  PurgeTerrSpec complete."
~/sysprocs/LOGMSG "  PurgeTerrSpec complete."
#end proc
