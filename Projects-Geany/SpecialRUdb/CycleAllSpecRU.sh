#!/bin/bash
echo " ** CycleAllSpecRU.sh out-of-date **";exit 1
echo " ** CycleAllSpecRU.sh out-of-date **";exit 1
# 9/23/22.    wmk.   (automated) CB *codebase env var support.
# 10/4/22.    wmk.   (automated) fix *pathbase for CB system.
# CycleAllSpecRU.sh = Update RU downloads from list.
#	5/31/22.	wmk.
#
# Usage.	bash CycleAllSpecRU.sh
#
...
# 5/31/22.	wmk.	single quotes removed from file= in do.
# Legacy mods.
...
#proc body here
pushd ./ >> $TEMP_PATH/bitbucket.txt
cd $codebase/Projects-Geany/SpecialRUdb
MY_PROJ='Territories'
echo "  CycleAllSpecRU beginning processing."
error_counter=0		# set error counter to 0
IFS="&"			# set & as the word delimiter for read.
file=DBList.txt
if ! test -f $file;then
 echo "** CycleAllSpecRU missing file DBList.txt - abandoned. **"
 ~/sysprocs/LOGMSG "   CycleAllSpecRU missing file DBList.txt - abandoned."
 exit 1
fi
i=0
while read -e; do
...
#  echo -e "  $firstchar\n is first char of line." >> $HOME/temp/scratchfile
  ...
  if [ "$firstchar" = "#" ]; then			# skip comment
   echo $REPLY >> $pathbase/temp/scratchfile
  else
   db_name=${REPLY:0:len}
   ./CycleSpecialRU.sh $db_name
...
# end proc body
bash ~/sysprocs/LOGMSG " CycleAllSpecRU complete." >> $system_log
echo " 	CycleAllSpecRU complete."
# end CycleAllSpecRU.sh
