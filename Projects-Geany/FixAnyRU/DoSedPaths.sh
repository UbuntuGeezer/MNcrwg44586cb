#!/bin/bash
echo " ** DoSedPaths.sh out-of-date **";exit 1
echo " ** DoSedPaths.sh out-of-date **";exit 1
# 9/23/22.    wmk.   (automated) CB *codebase env var support.
#DoSedPaths.sh - use sed to change all .sql files like FixXXXRU.sql.
#	4/25/22.	wmk.
#
#	Usage.	bash DoSedPaths.sh
#
# Entry.  TIDList.txt contains list of territory IDs to change .sql
#			files in.
#
# Modification History.
# ---------------------
# 5/28/21.	wmk.	original code.
# 4/25/22.	wmk.	generalized for FL/SARA/86777.
#
function jumpto
{
    label=$1
...
#  echo -e "  $firstchar\n is first char of line." >> $HOME/temp/scratchfile
  ...
  if [ "$firstchar" = "#" ]; then			# skip comment
   ...
   echo "s?/media/ubuntu/Windows/Users/Bill?%pathbase?g" > sedative.txt
   FBASE="Fix$TID"
   FN=$FBASE$FIX_SUFFX
   echo "  processing $FN"
   sed -i -f sedative.txt $FN
