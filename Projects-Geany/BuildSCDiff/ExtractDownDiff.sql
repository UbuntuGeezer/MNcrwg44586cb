...
-- *		AcctsAll - table of property IDs in N Venice territory
-- *
...
-- *			 support.
-- * 5/26/22.	wmk.	bug fixes; errant # comment line; AcctsNVen table
-- *			 reference removed.
-- * 6/16/23.	wmk.	default entry made in Diffmmdd for FOREIGN key
-- *			 linking with DiffAccts table.
-- * Legacy mods.
...
INSERT INTO $DIFF_TBL("Account#",Owner1,"SitusAddress(PropertyAddress)",
 HomesteadExemption)
VALUES('9999999999','SYSTEM - DEFAULT LINKING RECORD','0   NO ADDRESS','NO');
...
INSERT OR IGNORE INTO $DIFF_TBL
 SELECT $SCPA_TBL2.* FROM db15.$SCPA_TBL2
   ...
   INNER JOIN db8.AcctsAll
    ON db15.$SCPA_TBL2."Account#" = db8.AcctsAll."Account"
...
WITH a AS (SELECT Account FROM AcctsAll),
