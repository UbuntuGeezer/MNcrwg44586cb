#!/bin/bash
echo " ** DNCSummary.sh out-of-date **";exit 1
echo " ** DNCSummary.sh out-of-date **";exit 1
# 9/23/22.    wmk.   (automated) CB *codebase env var support.
#hdrAnySQL.sh - Any .sql to .sh shell source.
# 4/23/22.	wmk.
#	Usage. bash DNCSummary.sh
#		
# Dependencies.
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  DNCSummary - initiated from Make"
  echo "  DNCSummary - initiated from Make"
else
  ~/sysprocs/LOGMSG "  DNCSummary - initiated from Terminal"
  echo "  DNCSummary - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
echo "-- *"  >> SQLTemp.sql
echo "-- * Exit.  *codebase/Projects-Geany/DoTerrsWithCalc/Tracking/DNCSummary.csv"  >> SQLTemp.sql
...
echo ".output '$codebase/Projects-Geany/DoTerrsWithCalc/Tracking/DNCSummary.csv'"  >> SQLTemp.sql
...
sqlite3 < SQLTemp.sql
echo "  DNCSummary complete."
~/sysprocs/LOGMSG "  DNCSummary complete."
#end proc
