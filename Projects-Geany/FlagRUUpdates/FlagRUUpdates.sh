...
# 6/5/23.	wmk.	check for OBSOLETE territory added.
#
...
#procbodyhere
	...
	while read -e;do
	 ...
	 if test -f $pathbase/$rupath/Terr$TID/OBSOLETE;then \
	  echo " ** Territory yyy OBSOLETE - FlagRUUpdates skipping... **";skipit=1;fi
	 if [ $skipit -eq 0 ];then
	  ...
	  if test -f $TEMP_PATH/counter;then read -e < $TEMP_PATH/counter; \
	   ...
	   if [ $anum -gt 0 ];then echo "$TID" >> $projpath/RUtidList.txt;fi;fi;fi
	done < $file
