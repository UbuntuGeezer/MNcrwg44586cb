...
if [ -z "$P1" ] || [ -z "$P2" ] || [ -z "$P3" ] || [ -z "$P4" ]; then
 echo " Must specify m1 d1 m2 d2.. DoSed terminated."
 exit 1
fi
if [ -z "${terrdflt}" ];then
...
sed -f sedative.txt BuildDiffAccts.psq > BuildDiffAccts.sql
if test -f ExtractDownDiff.psq;then
 sed -f sedative.txt ExtractDownDiff.psq > ExtractDownDiff.sql
 echo "*sed* built ExtractDownDiff.sql..."
fi
if test -f NoTerrIDs.psq;then
 sed -f sedative.txt NoTerrIDs.psq > NoTerrIDs.sql
 echo "*sed* built NoTerrIDs.sql..."
fi
sed -f sedative.txt MakeExtractDiff.tmp > MakeExtractDiff
sed -f sedative.txt MakeBuildDiffAccts.tmp > MakeBuildDiffAccts
echo "  DoSed complete."
