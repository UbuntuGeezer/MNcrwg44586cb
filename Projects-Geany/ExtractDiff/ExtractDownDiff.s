...
echo "-- *	SCPADiff0630.db - as main, new differences database to populate" >> SQLTemp.sql
echo "-- *	SCPA_0526.db - as db14, SCPA (old) full download from date 05/26/2050" >> SQLTemp.sql
echo "-- *		Data0526 - SCPA download records from date 05/26/2050" >> SQLTemp.sql
echo "-- *	SCPA_0630.db - as db15, SCPA (new) full download from date 05/26/2050" >> SQLTemp.sql
echo "-- *		Data0630 - SCPA download records from date 06/30/2050" >> SQLTemp.sql
...
echo "-- *	SCPA_DB1 = SCPA_05-26.db with 05 26 substituted " >> SQLTemp.sql
echo "-- * 	SCPA_TBL1 = Data0526 with 0526 substituted" >> SQLTemp.sql
echo "-- *	SCPA_DB2 = SCPA_06-30.db with 06 30 substituted" >> SQLTemp.sql
echo "-- *	DIFF_DB = SCPADiff_06-30.db with 06 30 substituted" >> SQLTemp.sql
echo "-- *	DIFF_TBL = Diff0630 with 06 30 substituted" >> SQLTemp.sql
echo "-- *	M2D2 = \"06-30\" with 06 30 substituted" >> SQLTemp.sql
echo "-- *" >> SQLTemp.sql
echo "-- * exit DB and table results." >> SQLTemp.sql
echo "-- *	SCPADiff_0630.db - as main populated with tables and records" >> SQLTemp.sql
echo "-- *		Diffmmdd - table of new records that differ from previous" >> SQLTemp.sql
echo "-- *		  download; date stamped with yyyy-06-30 in records" >> SQLTemp.sql
...
echo "-- * 4/26/22.	wmk.	completed .sql ExtractDownDiff0630.sql migrated" >> SQLTemp.sql
echo "-- *			 into /86777/..ExtractDiff project." >> SQLTemp.sql
echo "-- * Legacy mods." >> SQLTemp.sql
echo "-- * 9/30/20.	wmk.	original query." >> SQLTemp.sql
echo "-- * 12/3/20.	wmk.	modified to use generic SCPA_05-26.db as db14 for" >> SQLTemp.sql
echo "-- *			 old SCPA full download, SCPA_06-30.db as db15 for" >> SQLTemp.sql
echo "-- *			 new SCPA full download to facilitate conversion" >> SQLTemp.sql
echo "-- *			 to QSCPADiff.sh shell script" >> SQLTemp.sql
echo "-- * 2/3/21.	wmk.	comments updated; mod to use SCPA_0526.db as db14" >> SQLTemp.sql
echo "-- *			 SCPA_0630.db as db15 as new name support; all" >> SQLTemp.sql
...
echo "-- *			 new Diff0630 table, and added via ALTER TABLE " >> SQLTemp.sql
echo "-- *			 after loading difference records." >> SQLTemp.sql
echo "-- * 6/19/21.	wmk.	multihost support modifications." >> SQLTemp.sql
echo "-- * 7/22/21.	wmk.	temporary modification since 0630 field names" >> SQLTemp.sql
echo "-- *				 re trimmed (automatic import), but 0526 field" >> SQLTemp.sql
...
echo "-- * 3/19/22.	wmk.	manually edited for SCPA_03-19.db, SCPA_06-30.db" >> SQLTemp.sql
echo "-- *			 Data0319, Data0630." >> SQLTemp.sql
echo "-- *" >> SQLTemp.sql
echo "-- * Notes. This query differences 2 SCPA downloads into a new database" >> SQLTemp.sql
echo "-- * SCPADiff_06-30.db with table Diffmmdd. The records in Diffmmdd are" >> SQLTemp.sql
echo "-- * all those records in SCPA_06-30.db that differ from records in " >> SQLTemp.sql
echo "-- * SCPA_05-26.db.;" >> SQLTemp.sql
echo "" >> SQLTemp.sql
echo ".cd '$pathbase/RawData/SCPA/SCPA-Downloads'" >> SQLTemp.sql
echo ".open 'SCPADiff_06-30.db'" >> SQLTemp.sql
echo ".shell echo \"Differencing SCPA_05-26.db with SCPA_06-30.db..may take a few minutes\"" >> SQLTemp.sql
...
echo " " >> SQLTemp.sql
echo "-- *	SCPA_0630.db - as db15, SCPA (new) full download from date 05/26" >> SQLTemp.sql
echo "-- *		Data0630 - SCPA download records from date 06/30 any uear" >> SQLTemp.sql
echo "ATTACH '$pathbase'" >> SQLTemp.sql
echo " ||		'/RawData/SCPA/SCPA-Downloads/SCPA_06-30.db'" >> SQLTemp.sql
...
echo "" >> SQLTemp.sql
echo "-- create empty Diff0630 table in New database ...;" >> SQLTemp.sql
echo "DROP TABLE IF EXISTS Diff0630;" >> SQLTemp.sql
echo "CREATE TABLE Diff0630" >> SQLTemp.sql
...
echo "INSERT OR IGNORE INTO Diff0630" >> SQLTemp.sql
echo " SELECT Data0630.* FROM db15.Data0630" >> SQLTemp.sql
echo "   INNER JOIN db14.Data0526" >> SQLTemp.sql
echo "    ON db15.Data0630.\"Account#\" = db14.Data0526.\"Account#\"" >> SQLTemp.sql
echo "   INNER JOIN db8.AcctsAll" >> SQLTemp.sql
echo "    ON db15.Data0630.\"Account#\" = db8.AcctsAll.Account" >> SQLTemp.sql
echo "	WHERE db15.Data0630.\"LastSaleDate\" <> db14.Data0526.\"LastSaleDate\"" >> SQLTemp.sql
echo "	    OR db15.Data0630.\"HomesteadExemption(YESORNO)\"" >> SQLTemp.sql
echo "	       <> db14.Data0526.\"HomesteadExemption(YESorNO)\"" >> SQLTemp.sql
echo "	ORDER BY db14.Data0526.\"Account#\";" >> SQLTemp.sql
echo "" >> SQLTemp.sql
