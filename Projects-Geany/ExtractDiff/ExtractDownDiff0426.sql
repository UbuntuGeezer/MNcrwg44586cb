...
-- *	SCPADiff0426.db - as main, new differences database to populate
-- *	SCPA_0205.db - as db14, SCPA (old) full download from date 02/05/2020
-- *		Data0205 - SCPA download records from date 02/05/2020
-- *	SCPA_0426.db - as db15, SCPA (new) full download from date 02/05/2020
-- *		Data0426 - SCPA download records from date 04/26/2020
...
-- *	SCPA_DB1 = SCPA_02-05.db with 02 05 substituted 
-- * 	SCPA_TBL1 = Data0205 with 0205 substituted
...
-- *	SCPADiff_0426.db - as main populated with tables and records
-- *		Diffmmdd - table of new records that differ from previous
-- *		  download; date stamped with yyyy-04-26 in records
...
-- *			 of AuxSCPAData/NVenAccts; 02 05 forced to 02 05.
-- * 4/26/22.	wmk.	completed .sql ExtractDownDiff0426.sql migrated
-- *			 into /86777/..ExtractDiff project.
-- * Legacy mods.
-- * 9/30/20.	wmk.	original query.
-- * 12/3/20.	wmk.	modified to use generic SCPA_02-05.db as db14 for
-- *			 old SCPA full download, SCPA_04-26.db as db15 for
-- *			 new SCPA full download to facilitate conversion
-- *			 to QSCPADiff.sh shell script
-- * 2/3/21.	wmk.	comments updated; mod to use SCPA_0205.db as db14
-- *			 SCPA_0426.db as db15 as new name support; all
...
-- *			 new Diff0426 table, and added via ALTER TABLE 
-- *			 after loading difference records.
-- * 6/19/21.	wmk.	multihost support modifications.
-- * 7/22/21.	wmk.	temporary modification since 0426 field names
-- *				 re trimmed (automatic import), but 0205 field
-- *			 names are not...!!!
...
-- * SCPADiff_04-26.db with table Diffmmdd. The records in Diffmmdd are
-- * all those records in SCPA_04-26.db that differ from records in 
-- * SCPA_02-05.db.;
...
-- *	SCPA_0426.db - as db15, SCPA (new) full download from date 02/05
-- *		Data0426 - SCPA download records from date 04/26 any uear
ATTACH '$pathbase'
...
-- create empty Diff0426 table in New database ...;
