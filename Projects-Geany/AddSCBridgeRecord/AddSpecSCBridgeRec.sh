...
# 12/12/22.	wmk.
#	Usage. bash AddSpecSCBridgeRec.sh
#		
# Dependencies.
...
if [ -z "$system_log" ]; then
  ...
  ~/sysprocs/LOGMSG "  AddSpecSCBridgeRec - initiated from Make"
  echo "  AddSpecSCBridgeRec - initiated from Make"
else
  ~/sysprocs/LOGMSG "  AddSpecSCBridgeRec - initiated from Terminal"
  echo "  AddSpecSCBridgeRec - initiated from Terminal"
fi 
TEMP_PATH=$HOME/temp
#	Environment vars:
if [ -z "$TODAY" ];then
...
echo "-- * AddSpecSCBridgeRec.psq - AddSpecSCBridgeRec.sql template."  > SQLTemp.sql
echo "-- *	5/24/23.	wmk."  >> SQLTemp.sql
echo "-- *"  >> SQLTemp.sql
echo "-- * Modification History."  >> SQLTemp.sql
echo "-- * ---------------------"  >> SQLTemp.sql
echo "-- * 5/24/23.	wmk.	original code; adapted from AddSCBridgeRec.psq."  >> SQLTemp.sql
...
echo "-- *"  >> SQLTemp.sql
echo "-- * Notes. sed substitutes property id for w www, special-db "  >> SQLTemp.sql
echo "-- * for v vvvv, and <terrid> for 802 within this query, writing the resultant to .sql."  >> SQLTemp.sql
echo "-- *;"  >> SQLTemp.sql
echo ""  >> SQLTemp.sql
echo ".open '$pathbase/RawData/SCPA/SCPA-Downloads/Special/Aria.db'"  >> SQLTemp.sql
...
echo "INSERT OR REPLACE INTO Spec_SCBridge"  >> SQLTemp.sql
...
echo "END"  >> SQLTemp.sql
echo "WHERE OwningParcel IS '0391120115';"  >> SQLTemp.sql
echo ".quit"  >> SQLTemp.sql
echo "-- End AddSpecSCBridgeRec;"  >> SQLTemp.sql

#endprocbody - changed from end proc body 12/3/21 for awk reversal.
# jumpto references removed 9/6/21.
sqlite3 < SQLTemp.sql
echo "  AddSpecSCBridgeRec complete."
~/sysprocs/LOGMSG "  AddSpecSCBridgeRec complete."
#end proc
