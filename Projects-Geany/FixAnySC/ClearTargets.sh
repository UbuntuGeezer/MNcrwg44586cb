...
# ClearTargets - Clear targets for SC project to force rebuild.
#	4/25/22.	wmk.
#
...
# 9/21/21.	wmk.	modified for FixAnySC project.
P1=$1
TID=$1
P2=${2^^}
P3=${3^^}
...
P1=${P1,,}
echo "  ClearTargets (SC) $P1 initiated."
dirbase=Terr
fname=Fix
fnend=SC
...
#  echo -e "  $firstchar\n is first char of line." >> $HOME/temp/scratchfile
  ...
  if [ "$firstchar" = "#" ]; then			# skip comment
   ...
   TID=${REPLY:0:len}
   targpath=$pathbase/RawData/SCPA/SCPA-Downloads/$dirbase$TID
...
~/sysprocs/LOGMSG "  ClearTargets (SC) complete."
echo "  ClearTargets (SC) complete."
# end ClearTargets.sh
