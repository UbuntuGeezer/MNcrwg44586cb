#!/bin/bash
echo " ** NewRUSCTerrToDBs.sh out-of-date **";exit 1
...
# Legacy mods.
# 1/17/22.	wmk.	"U" type record support.
# Legacy mods.
...
#
P1=$1
~/sysprocs/LOGMSG "  NewRUSCTerrToDBs $P1 (Dev) started."
...
echo "WHERE RecordType IS \"P\"" >> SQLTemp.sql
echo "   OR RecordType IS \"U\"" >> SQLTemp.sql
echo "AND DelPending IS NOT 1" >> SQLTemp.sql
echo " ;" >> SQLTemp.sql
...
sqlite3 < SQLTemp.sql
 ...
 echo "** sqlite3 skipped - source code on $PWD/SQLTemp.sql **" 
fi
#
popd > /dev/null
~/sysprocs/LOGMSG "  NewRUSCTerrToDBs $P1 (Dev) $TST_STR complete."
