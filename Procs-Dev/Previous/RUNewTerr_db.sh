...
#	7/18/22.	wmk. (Dev)
# RUNewTerr_db - run sqlite processing raw download data into db
...
if [ $NO_PROMPT = 0 ]; then
  ...
  if [ $REPLY == 'y' ] || [ $REPLY == 'Y' ]; then
   echo "  Proceeding with RUNewTerr_db..."
  else
   echo "  RUNewTerr_db - user wants to delete headings in .csv" >> $system_log
   echo "  RUNewTerr_db abandoned - delete headings in .csv"
   exit 0 
  fi
fi
